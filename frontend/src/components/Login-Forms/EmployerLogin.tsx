...
// This is the employer login form that will display when the user selects "Employer" on the login page. 
...
const EmployerLogin: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      try {
        // Simulate login request (replace with actual login API)
        await new Promise((resolve) => setTimeout(resolve, 1000)); // Fake delay
        setIsSuccess(true);
        navigate("/employer"); // Navigate only after success
      } catch (err) {
